...
const DistributionView: React.FC<DistributionViewProps> = ({ data }) => {
  if (!data || data.length === 0) {
    return (
      <div className="glass-card p-6 h-full flex flex-col items-center justify-center text-slate-500 gap-4">
    ...
    <div className="glass-card p-6 relative overflow-hidden">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-xs text-slate-400 uppercase tracking-widest mt-1">Real-time Delivery Network</p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {data.map((event, i) => (
          <motion.div 
            ...
            className="flex items-center gap-4 bg-white/2 p-4 rounded-2xl border border-white/5 relative group hover:bg-white/5 transition-all"
          >
            ...
            <div className="flex-1">
              <div className="flex justify-between items-end mb-1">
                <span className="text-sm font-bold text-white">{event.home}</span>
      ...
      <div className="mt-8 pt-6 border-t border-slate-800/50 flex justify-between items-center text-[10px] font-bold text-slate-500 uppercase tracking-tighter">
        <div className="flex items-center gap-2">
